...
def find_output_dir_appr(relative_folder: str = "") -> tuple[Path, Path]:
    """
    ...
    if relative_folder:
        ...
        super_res_dir = Path(OUTPUT_IMAGES_APPR_DIR) / f"sr_{final_super_res_suffix}"
        downscaling_dir = Path(OUTPUT_IMAGES_APPR_DIR) / f"downscaled_{final_super_res_suffix}"   
...
def find_output_dir(relative_folder: str = "") -> tuple[Path, Path]:
    """
    Determine the appropriate super-resolution output directory based on the scale factor.
